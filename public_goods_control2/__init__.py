...
class Player(BasePlayer):
    ...
    min=0, max=C.ENDOWMENT, label="你选择投入公共池的代币数为（0–20之间任意整数）：")
...
class Instructions(Page):
    ...
    def error_message(player: Player, values):
        """验证答案是否正确"""
        if values.get('comprehension_answer') != 24:
            # 使用 participant.vars 跟踪错误次数（在 error_message 中不能直接保存 player）
            ...
            if error_count < 3:
                ...
                # 第三次及以后显示详细提示
                return '答案不正确，请重新计算。提示：A保留19个代币，公共池总额10个代币翻倍后为20个，平均每人分5个，所以A最终有19+5=24个代币。'
...
class ComprehensionCheck(Page):
    ...
    def error_message(self, values):
        ...
        # 检查问题3：最终获得代币数，正确答案为20
        if values.get('comprehension_q3') != 20:
...
class ComprehensionWaitPage(WaitPage):
    """理解检验后等待页面"""
    title_text = "等待其他玩家"
    body_text = "请稍候，等待其他玩家完成理解检验并进入游戏..."
